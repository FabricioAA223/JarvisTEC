...
import CarForm, { modeloCoche } from "./CarForm";
...
const CATEGORIAS = [
  ...
  {
    keywords: ['precedir el precio de un automovil', 'precio de un automóvil', 'predecir precio automóvil','precedir el precio de un vehículo', 'precio de un vehículo', 'predecir precio vehículo', 'precio vehículo', 'vehículo', 'precio auto', 'precio de un auto', 'auto'], 
    nombre:"Precio auto", 
...
const HomePage = ({ route }) => { //: React.FC<HomePageProps>
  ...
  useEffect(() => {
    const sayMyName = async () => {
      await speakAndWait(`Bienvenido jefe ${name}`)
  ...
  const sendAudioToBackend = async (uri, retry) => {
    ...
    try {
      const response = await fetch('http://172.24.95.117:8000/convert-and-transcribe/', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const callIAModel = async (data, endPoint) => {
    ...
    try {
      const response = await fetch(`http://172.24.95.117:8080/${endPoint}`, {
        method: 'POST',
